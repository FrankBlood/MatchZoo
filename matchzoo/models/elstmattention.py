...
class ELSTMAttention(BasicModel):
    def __init__(self, config):
        super(ELSTMAttention, self).__init__(config)
        self.__name = 'ELSTMAttention'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[ELSTMAttention] parameter check wrong')
        print('[ELSTMAttention] init done', end='\n')
