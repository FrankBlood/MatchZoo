...
class BiLSTM(BasicModel):
    def __init__(self, config):
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[BiLSTM] parameter check wrong')
        print('[BiLSTM] init done', end='\n')
    ...
    def build(self):
        ...
        rnn = Bidirectional(LSTM(units=self.config['hidden_size'], dropout=self.config['dropout_rate'],
