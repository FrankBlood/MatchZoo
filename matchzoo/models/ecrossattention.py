...
class ECrossAttention(BasicModel):
    def __init__(self, config):
        super(ECrossAttention, self).__init__(config)
        self.__name = 'ECrossAttention'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[ECrossAttention] parameter check wrong')
        print('[ECrossAttention] init done', end='\n')
