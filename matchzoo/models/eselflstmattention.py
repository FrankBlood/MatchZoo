...
class ESelfLSTMAttention(BasicModel):
    def __init__(self, config):
        super(ESelfLSTMAttention, self).__init__(config)
        self.__name = 'ESelfLSTMAttention'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[ESelfLSTMAttention] parameter check wrong')
        print('[ESelfLSTMAttention] init done', end='\n')
