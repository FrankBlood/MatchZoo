...
class ESelfAttention(BasicModel):
    def __init__(self, config):
        super(ESelfAttention, self).__init__(config)
        self.__name = 'ESelfAttention'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[ESelfAttention] parameter check wrong')
        print('[ESelfAttention] init done', end='\n')
