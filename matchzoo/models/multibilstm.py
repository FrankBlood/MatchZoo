...
class MultiBiLSTM(BasicModel):
    def __init__(self, config):
        super(MultiBiLSTM, self).__init__(config)
        self.__name = 'MultiBiLSTM'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[MultiBiLSTM] parameter check wrong')
        print('[MultiBiLSTM] init done', end='\n')
    ...
    def build(self):
        ...
        rnn_with_hidden = Bidirectional(LSTM(units=self.config['hidden_size'], dropout=self.config['dropout_rate'],
                                             recurrent_dropout=self.config['dropout_rate'], return_sequences=True))
        rnn = Bidirectional(LSTM(units=self.config['hidden_size']*2, dropout=self.config['dropout_rate'],
