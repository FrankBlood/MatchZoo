...
class EAttention(BasicModel):
    def __init__(self, config):
        super(EAttention, self).__init__(config)
        self.__name = 'EAttention'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[EAttention] parameter check wrong')
        print('[EAttention] init done', end='\n')
